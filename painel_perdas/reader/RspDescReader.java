package br.com.bb.disec.bean.reader;

import java.sql.*;
import br.com.bb.disec.bean.*;
import br.com.bb.disec.bean.iface.*;

/**
 * BeanReader utility class to generate RspDesc
 * JavaBean from a given java.sql.ResultSet.
 */
public class RspDescReader {

  private final ResultSet rset;

  /**
   * Default constructor receiving the ResultSet.
   * @param rs ResultSet containing the RspDesc information.
   */
  public RspDescReader( ResultSet rs ) {
    if(rs == null) {
      throw new IllegalArgumentException("ResultSet must be not null");
    }
    this.rset = rs;
  }

  /**
   * Get the ResultSet of this BeanReader.
   * @return java.sql.ResultSet.
   */
  public ResultSet getResultSet() {
    return this.rset;
  }

  /**
   * Identifies if the ResultSet contains a column with the given name.
   * @param col The column name to be verified.
   * @return [true] if the ResultSet contains the column, [false] otherwise.
   */
  private boolean contains( String col ) {
    try {
      return rset.findColumn( col ) > 0;
    } catch( SQLException e ) {
      return false;
    }
  }

  /**
   * Create a RspDesc bean with the ResultSet information.
   * @return The created RspDesc bean.
   */
  public IRspDesc readBean() throws SQLException {
    IRspDesc bean = new RspDesc();
    if(this.contains( IRspDesc.COLUMN_TX_RSP )) {
      bean.setTxRsp( rset.getString( IRspDesc.COLUMN_TX_RSP) );
    }
    if(this.contains( IRspDesc.COLUMN_NM_RSP )) {
      bean.setNmRsp( rset.getString( IRspDesc.COLUMN_NM_RSP) );
    }
    return bean;
  }

}